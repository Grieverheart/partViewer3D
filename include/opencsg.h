...
#ifdef DLL_EXPORT
#define CSGAPI __declspec(dllexport)
#else
#define CSGAPI __declspec(dllimport)
#endif
...
namespace OpenCSG {
    ...
    /// directly. 
    void CSGAPI render(const std::vector<Primitive*>& primitives);
    ...
    void CSGAPI setOption(OptionType option, int newSetting);
    /// Returns the current setting of the provided option type.
    int  CSGAPI getOption(OptionType option);
    ...
    void CSGAPI setContext(int context);
    /// Returns the current context.
    int CSGAPI getContext();
    /// Releases the OpenGL resources allocated by OpenCSG for the current
    /// context. 
    void CSGAPI freeResources();
